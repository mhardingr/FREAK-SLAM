...
double FFREAK::distance(const FFREAK::TDescriptor &a, 
...
void FFREAK::toMat32F(const cv::Mat &descriptors, cv::Mat &mat)
{
...
void FFREAK::toMat8U(const std::vector<TDescriptor> &descriptors, 
  cv::Mat &mat)
{
  mat.create(descriptors.size(), FFREAK::L, CV_8U);
  
  unsigned char *p = mat.ptr<unsigned char>();
  
  for(size_t i = 0; i < descriptors.size(); ++i, p += FFREAK::L)
  {
    const unsigned char *d = descriptors[i].ptr<unsigned char>();
    std::copy(d, d + FFREAK::L, p);
  }
